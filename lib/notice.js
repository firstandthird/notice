  ...
  $.notice = function(message, options) {
    ...
    var close = function() {
      ...
      if(opts.anchor === 'top') {
        ...
        $('.notice').animate({ opacity: 0, bottom: '-='+animationOffset }, {
          complete: function() {
            $(this).remove();
          }
        });
    ...
    if(opts.anchor === 'top') {
      el.css({
        ...
        top: (opts.offsetTop || containerOffset.top) - animationOffset
      })
      .animate({ opacity: 0.9, top: '+='+animationOffset });
    } else {
      el.css({
        left: ((container.width() - el.width()) / 2) + containerOffset.left,
        bottom: opts.offsetBottom - animationOffset
      })
      .animate({ opacity: 0.9, bottom: '+='+animationOffset });
    }
